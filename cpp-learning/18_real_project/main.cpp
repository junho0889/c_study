...
int main() {
    ...
    service.show_all();
    // → 정렬: 미완료 → 우선순위 높은 순
    //   id=1 HIGH, id=2 MEDIUM, id=3 LOW (모두 미완료)
    // > 출력 (대략):
    //   ─────────── 할 일 목록 ───────────
    //     [ ] #1 [높음] C++ 01~06단계 복습 — 변수, 제어문, 함수, OOP  (2026-05-06 ...)
    //     [ ] #2 [보통] 디자인 패턴 공부 — 싱글톤, 팩토리, 옵저버  (2026-05-06 ...)
    //     [ ] #3 [낮음] 개인 프로젝트 시작 — TODO 앱 확장하기  (2026-05-06 ...)
    //   ──────────────────────────────────
    //   전체: 3  완료: 0  남음: 3

    service.add("CMake 배우기", "빌드 시스템 학습", Priority::MEDIUM);
    // → id=4 추가
    // > 출력:   [알림] 새 할 일 추가: CMake 배우기
    service.show_all();
    // → 정렬: HIGH(1), MEDIUM(2), MEDIUM(4), LOW(3)
    //   같은 우선순위면 입력 순서 유지 (stable_sort 아니므로 약간 변동 가능)

    service.complete(1);
    // → id=1 toggle: completed false → true
    // > 출력:   [알림] 상태 변경: #1 → 완료
    service.show_all();
    // → 미완료 먼저 → MEDIUM(2), MEDIUM(4), LOW(3), 완료(1)
    //   전체: 4  완료: 1  남음: 3
